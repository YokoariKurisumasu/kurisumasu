...
public class candle : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (Spone.fireswitch == true || Input.GetKeyDown(KeyCode.Alpha7))
        {
           /* GameObject fire = GameObject.Find("Fire(Clone)");
            Vector3 firePos = fire.transform.position;
            */
        ...
        if (Spone.fireswitch == true || Input.GetKeyDown(KeyCode.Alpha8))
        {
            //二本目のキャンドルと手の座標を比べる
        ...
        {
            if (Mathf.Sqrt(len) <= 1.0f && FindObjectOfType<Balloon>().cd_balloon == true
